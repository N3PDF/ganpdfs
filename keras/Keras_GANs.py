...
pdfs_init   = pdf.mkPDFs()
pdf_central = sample(pdfs_init,20)
...
def sample_pdf(n):
    ...
    for x in x_pdf:
        ...
        for p in pdf_central:
            row.append(p.xfxQ2(2,x,Q_pdf)-p.xfxQ2(-2,x,Q_pdf))
        data.append(row)
...
Generator = Sequential([
    Dense(16, input_dim=random_noise_dim),
    LeakyReLU(0.2),
    Dense(32),
    LeakyReLU(0.2),
    Dense(length)
...
for k in range(1,number_training+1):
    noise = np.random.normal(0,1,size=[1,random_noise_dim])
    pdf_batch = [pdf_data]
